	...
	while ($arProp = $rsProp->Fetch())
	{
		if(in_array($arProp["CODE"],$arParams["PROPERTY_CODE"]) && $arProp["PROPERTY_TYPE"] != "F")
		{
			$arTemp = array(
				...
				//no multiple values!
				"MULTIPLE" => false, //$arProp["MULTIPLE"],
			);
            ...
            {
                //data from another iblock
                if($arProp['PROPERTY_TYPE'] == 'E' && !empty($arProp['LINK_IBLOCK_ID']))
                {

                    $arTemp['LIST_TYPE'] = $arTemp['PROPERTY_TYPE'] = 'L';
                    ...
                    while($row = $res->fetch())
                        $arTemp["VALUE_LIST"][$row['ID']] = $row['NAME'];
                }
            }
...
foreach($arResult["arrProp"] as $prop_id => $arProp)
{
    ...
    if($arProp['PROPERTY_TYPE'] == 'N' && !in_array($arProp['CODE'], array('PRICE')))
    {
