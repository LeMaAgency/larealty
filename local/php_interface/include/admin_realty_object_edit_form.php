        ...
        <td>
            ...
            <? else: ?>
                ...
                if ($str_PREVIEW_TEXT_TYPE != "html") echo " checked" ?>> <label for="PREVIEW_TEXT_TYPE_text"><?
                    echo GetMessage("IBLOCK_DESC_TYPE_TEXT") ?></label> / <input type="radio" name="PREVIEW_TEXT_TYPE"
...
if ($arShowTabs['bizproc']):
        ...
        <tr class="heading">
            <td colspan="2">
                ...
                if (strlen($arDocumentState["ID"]) > 0 && strlen($arDocumentState["WORKFLOW_STATUS"]) > 0):?>
                    (<a href="<?
                    echo htmlspecialcharsbx("/bitrix/admin/" . CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array(
                            "WF" => $WF,
                            "find_section_section" => $find_section_section,
            ...
            <tr>
                ...
                <td width="60%"><?
                    if (strlen($arDocumentState["ID"]) > 0): ?><a
                            href="/bitrix/admin/bizproc_log.php?ID=<?= $arDocumentState["ID"] ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>"><?
                        endif;
                ...
                <tr>
                    ...
                    <td width="60%">
                        ...
                        foreach ($arTasks as $arTask) {
                            ?><a
                            href="/bitrix/admin/bizproc_task.php?id=<?= $arTask["ID"] ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>"
    ...
    if ($ID > 0):
        ...
        if ($bStartWorkflowPermission):
            ...
            <tr>
                <td colspan="2" align="center">
                    <a href="/bitrix/admin/<?= MODULE_ID ?>_start_bizproc.php?document_id=<?= $ID ?>&document_type=<?= DOCUMENT_TYPE ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array('bxpublic'))) ?>"><?
    ...
    $(function () {
        ...
        $('#tr_SECTIONS select[name="IBLOCK_SECTION[]"]').on('change', function (e) {
            ...
            switch (String(realtyTypes[realtyType])) {
                ...
                case '1':
                    sortProps = {
                        '#tr_edit1_csection4': [29, 13, 30, 31, 32, 33, 34], //Адрес
                        '#tr_edit1_csection1': [2, 7, 19, 9, 6, 5, 4, 22, 23, 8, 21, 35, 24, 25], //Характеристики
                        '#tr_edit1_csection2': [38, 77, 39, 78, 26, 27, 43], //Наличие
                        '#tr_edit1_csection3': [3, 79, 107], //Условия
                        '#tr_edit1_csection5': [68, 69, 70, 71, 54, 72, 73] //Собственник
                    };
                    checkProperty = [29, 13, 30, 31, 32, 33];
                    break;
                //Комнаты
                case '2':
                    sortProps = {
                        '#tr_edit1_csection4': [29, 13, 30, 31, 32, 33, 34], //Адрес
                        '#tr_edit1_csection1': [2, 86, 7, 19, 9, 6, 5, 4, 8, 21, 24, 25, 57], //Характеристики
                        '#tr_edit1_csection2': [38, 77, 39, 78, 26, 43, 27], //Наличие
                        '#tr_edit1_csection3': [3, 107, 79], //Условия
                        '#tr_edit1_csection5': [68, 69, 70, 71, 54, 72, 73] //Собственник
                    };
                    checkProperty = [29, 13, 30, 31, 32, 33];
                    break;
                ...
                case '152':
                    sortProps = {
                        '#tr_edit1_csection4': [29, 13, 30, 31, 34, 85], //Адрес
                        '#tr_edit1_csection1': [2, 5, 19, 9, 4, 22, 23, 36, 21, 35, 24, 25], //Характеристики
                        '#tr_edit1_csection2': [38, 77, 39, 42, 40, 81, 41, 43, 27, 48, 49, 45, 44], //Наличие
                        '#tr_edit1_csection3': [3, 79, 107, 82, 83], //Условия
                        '#tr_edit1_csection5': [68, 69, 70, 71, 54, 72, 73] //Собственник
                    };
                    checkProperty = [29, 13, 30, 31];
                    break;
                //Земельный участок
                case '4':
                    sortProps = {
                    ...
                    checkProperty = [29, 13, 30, 70];
                    break;
                //Офисы
                case '49':
                    sortProps = {
                        '#tr_edit1_csection4': [29, 13, 108, 30, 31, 32], //Адрес
                        '#tr_edit1_csection1': [58, 19, 9, 6, 5, 4, 59], //Характеристики
                        '#tr_edit1_csection2': [38, 77, 39, 20, 27, 47, 48, 51, 49], //Наличие
                        '#tr_edit1_csection3': [3, 79], //Условия
                        '#tr_edit1_csection5': [68, 69, 70, 71, 54, 72, 73] //Собственник
                    };
                    checkProperty = [29, 13, 30, 31, 32];
                    break;
                //Торговые площади
                case '50':
                    sortProps = {
                        '#tr_edit1_csection4': [29, 13, 108, 30, 31, 32, 85], //Адрес
                        '#tr_edit1_csection1': [19, 9, 6, 5, 4, 36, 59], //Характеристики
                        '#tr_edit1_csection2': [38, 77, 39, 43, 26, 47, 48, 51, 49], //Наличие
                        '#tr_edit1_csection3': [3, 79], //Условия
                        '#tr_edit1_csection5': [68, 69, 70, 71, 54, 72, 73] //Собственник
                    };
                    checkProperty = [29, 13, 30, 31, 32];
                ...
                case '51':
                    sortProps = {
                        '#tr_edit1_csection4': [29, 13, 108, 30, 31, 85], //Адрес
                        '#tr_edit1_csection1': [19, 9, 5, 4, 36], //Характеристики
                        '#tr_edit1_csection2': [38, 77, 39, 40, 52, 53, 41, 42, 43, 26, 46, 47, 48, 51, 49], //Наличие
                        '#tr_edit1_csection3': [3, 79], //Условия
                        '#tr_edit1_csection5': [68, 69, 70, 71, 54, 72, 73] //Собственник
                    };
                    checkProperty = [29, 13, 30, 31];
            ...
            /**
             * Add the class to the required properties. Prematurely deleting an existing class
             */
            $('.js-check-similar-address').removeClass('js-check-similar-address').attr('data-chech-similat-address-id', '');
            for (var idCheckProperty in checkProperty) {
                $('#tr_PROPERTY_' + checkProperty[idCheckProperty]).addClass('js-check-similar-address').attr('data-chech-similat-address-id', checkProperty);
            }
            ...
            /**
             * Output of the message, when there are equally filled fields for different elements
             */
            $('.js-check-similar-address').on('change', function (e) {
                e.preventDefault();
                var arSimilarAddress = $(this).data('chech-similat-address-id').split(','),
                    checkNoEmpty = true,
                    addressThisElem = [];

                /*Check for the absence of emptiness in the fields to be checked and
                 write values to the array "addressThisElem" if they are full*/

                for (var id in arSimilarAddress) {
                    var inputOrSelect = 'input';
                    if (arSimilarAddress[id] == '29' || arSimilarAddress[id] == '13') {
                        inputOrSelect = 'select';
                    }
                    if ($('#tr_PROPERTY_' + arSimilarAddress[id] + ' ' + inputOrSelect).val() == '') {
                        checkNoEmpty = false;
                        break;
                    } else {
                        addressThisElem[arSimilarAddress[id]] = $('#tr_PROPERTY_' + arSimilarAddress[id] + ' ' + inputOrSelect).val();
                    }
                }
                if (checkNoEmpty) {
                    ...
                    while ($ar_res = $res->Fetch()) {
                        isset($ar_res['PROPERTY_REGION_VALUE']) ? ($arAddress[$ar_res['ID']][13] = $ar_res['PROPERTY_REGION_ENUM_ID']) : '';
                        isset($ar_res['PROPERTY_CITY_VALUE']) ? ($arAddress[$ar_res['ID']][29] = $ar_res['PROPERTY_CITY_ENUM_ID']) : '';
                        isset($ar_res['PROPERTY_STREET_VALUE']) ? ($arAddress[$ar_res['ID']][30] = $ar_res['PROPERTY_STREET_VALUE']) : '';
                        isset($ar_res['PROPERTY_HOUSE_NUMBER_VALUE']) ? ($arAddress[$ar_res['ID']][31] = $ar_res['PROPERTY_HOUSE_NUMBER_VALUE']) : '';
                    ...
                    var arElementsAddress = {
                        <?
                        foreach ($arAddress as $key => $address){?>
                        '<?=$key;?>': {
                            <?foreach($address as $keyValue => $value){?>
                            '<?=$keyValue;?>': ['<?=$value;?>'],
                            <?}?>
                        },
                        <?}?>
